...
using CoreBotWithTests.Tests.Common;
...
namespace CoreBotWithTests.Tests.Dialogs
{
    public class MainDialogTests : BotTestBase
    {
        ...
        {
            // Arrange
            var mockRecognizer = SimpleMockFactory.CreateMockLuisRecognizer<TechBookingRecognizer>(null, constructorParams: new Mock<IConfiguration>().Object);
        ...
        public async Task TaskSelector(string utterance, string intent, string invokedDialogResponse, string taskConfirmationMessage)
        {
            ...
            var mockLuisRecognizer = SimpleMockFactory.CreateMockLuisRecognizer<TechBookingRecognizer, TechBooking>(
                new TechBooking
                {
                    Intents = new Dictionary<TechBooking.Intent, IntentScore>
                    {
                        { Enum.Parse<TechBooking.Intent>(intent), new IntentScore() { Score = 1 } },
                    },
                    Entities = new TechBooking._Entities(),
                },
        ...
        public async Task ShowsUnsupportedCitiesWarning(string jsonFile, string expectedMessage)
        {
            ...
            var mockLuisResult = JsonConvert.DeserializeObject<TechBooking>(luisResultJson);
            var mockLuisRecognizer = SimpleMockFactory.CreateMockLuisRecognizer<TechBookingRecognizer, TechBooking>(
                mockLuisResult,
                new Mock<IConfiguration>().Object);
            mockLuisRecognizer.Setup(x => x.IsConfigured).Returns(true);
